...
void execute_itype_except_load(Instruction instruction, Processor *processor) {
    ...
    switch (instruction.itype.funct3) {
        ...
        case 0x1:
            // SLLI
            processor->R[instruction.itype.rd] = 
            ((Word)processor->R[instruction.itype.rs1]) << sign_extend_number(0x00000000 | (0x0000001f & instruction.itype.imm), 5);
            break;
        case 0x2:
            // STLI
        ...
        case 0x5:
            ...
            switch (instruction.itype.imm) {
                case 0x00:
                    processor->R[instruction.itype.rd] = (Word)instruction.itype.rs1 >> sign_extend_number((0x00000000 | (instruction.itype.imm & 0x0000001f)), 5);
                    break;
                case 0x20:
                    processor->R[instruction.itype.rd] = 
                    ((sWord)processor->R[instruction.itype.rs1]) >> 
...
void execute_load(Instruction instruction, Processor *processor, Byte *memory) {
    ...
    switch (instruction.itype.funct3) {
        case 0x0:
            // LB
            processor->R[instruction.itype.rd] = load(
                memory,
...
void execute_lui(Instruction instruction, Processor *processor) {
    ...
    offset |= instruction.utype.imm & 0x000fffff;
    processor->PC += 4;
    processor->R[instruction.utype.rd] = sign_extend_number(offset, 20) << 12;
...
void store(Byte *memory, Address address, Alignment alignment, Word value) {
    ...
    if(alignment == LENGTH_WORD) {
        ...
        memory[address + 2] = (Byte)((value & 0x0000ff00) >> 8);
        memory[address + 3] = (Byte)((value & 0x00ff0000) >> 16);
        memory[address + 4] = (Byte)((value & 0xff000000) >> 24);
